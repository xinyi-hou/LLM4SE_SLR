...
# 定义文件路径
UNIQUE_BIB = '../remove_duplicates/no_duplicates/science_direct_unique.bib'
OUTPUT_XLSX = 'science_direct/science_direct_output_llm2.xlsx'
...
def filter1():
    ...
    for index, row in df.iterrows():
        ...
        if df.loc[index, 'TITLE_CK'] == '' and df.loc[index, 'ABSTRACT_CK'] == '' and df.loc[
            index, 'KEYWORDS_CK'] == '':
...
with pd.ExcelWriter(OUTPUT_XLSX, engine='openpyxl') as writer:
    ...
    # # 过滤出TSE的条目
    # df_tse = df[
    #     df['abstract'].str.contains('Transactions on Software Engineering', case=False, na=False, regex=False) & (
    #                 df['EXCLUDE'] == '')]
    # df_tse.to_excel(writer, index=False, sheet_name='TSE Entries')
    #
    # # 过滤出TIFS的条目
    # df_tifs = df[
    #     df['abstract'].str.contains('Transactions on Information Forensics and Security', case=False, na=False, regex=False) & (
    #                 df['EXCLUDE'] == '')]
