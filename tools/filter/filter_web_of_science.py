...
# 定义文件路径
UNIQUE_BIB = '../remove_duplicates/no_duplicates/web_of_science_unique.bib'
OUTPUT_XLSX = 'web_of_science/web_of_science_output_llm.xlsx'
...
# 标记无法计算页数的条目
df.loc[df['start_page'].isna() | df['end_page'].isna(), 'CHECK_PAGES'] += 'Pages error'
...
df = set_venue(df, 'booktitle', VENUES)
...
with pd.ExcelWriter(OUTPUT_XLSX, engine='openpyxl') as writer:
    ...
    #
    # # 过滤出ICSE的条目
    # df_icse = df[df['doi'].str.contains('ICSE', case=False, na=False, regex=False) & (df['EXCLUDE'] == '')]
    # df_icse.to_excel(writer, index=False, sheet_name='ICSE Entries')
    #
    # # 过滤出ISSTA的条目
    # df_issta = df[df['abstract'].str.contains('ISSTA', case=False, na=False, regex=False) & (df['EXCLUDE'] == '')]
    # df_icse.to_excel(writer, index=False, sheet_name='ISSTA Entries')
    #
    # # 过滤出ASE的条目
    # df_ase = df[df['doi'].str.contains('ASE', case=False, na=False, regex=False) & (df['EXCLUDE'] == '')]
